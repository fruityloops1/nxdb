...
Result pminfoAtmosphereGetProcessId(u64 *out_pid, u64 program_id) {
  return serviceDispatchInOut(pminfoGetServiceSession(), 65000, program_id,
                              *out_pid);
}
...
Result pminfoAtmosphereHasLaunchedBootProgram(bool *out, u64 program_id) {
  u8 tmp = 0;
  Result rc =
      serviceDispatchInOut(pminfoGetServiceSession(), 65001, program_id, tmp);
  if (R_SUCCEEDED(rc) && out)
    *out = tmp & 1;
  return rc;
}

Result pminfoAtmosphereGetProcessInfo(NcmProgramLocation *loc_out,
                                      CfgOverrideStatus *status_out, u64 pid) {
  struct {
...
Result pmdmntAtmosphereGetProcessInfo(Handle *handle_out,
                                      NcmProgramLocation *loc_out,
                                      CfgOverrideStatus *status_out, u64 pid) {
  Handle tmp_handle;
...
Result pmdmntAtmosphereGetCurrentLimitInfo(u64 *out_cur, u64 *out_lim,
                                           u32 group, u32 resource) {
  const struct {
    u32 group;
    u32 resource;
  } in = {group, resource};
