...
#include "types.h"
#include "windows/DebuggingSession.h"
...
namespace nxdb {
    ...
    void MemoryMapMgr::MapBlock::unmap(Handle processHandle) {
        ...
        while (R_SUCCEEDED(svcQueryMemory(&info, &pageinfo, addr))) {
            ...
            Result rc = svcUnmapProcessMemory(reinterpret_cast<void*>(addr), processHandle, addr - startMapped + startOrig, info.size);
            nxdb::log("svcUnmapProcessMemory(%p, %d, %p, %p) = %x", reinterpret_cast<void*>(addr), processHandle, addr, info.size, rc);
