...
namespace hk::util {
    ...
    namespace detail {
        ...
        template <typename T>
        struct ReadDefault {
            static constexpr T read(const T* data, uintptr_t offset, void*) {
                return data[offset];
            }
        };

        template <typename T, class Read>
        constexpr u32 getBlock(const T* p, u32 i, void* userData) {
        ...
        class HashMurmurImpl {
            ...
            constexpr static u32 nblocks(u32 len) { return len / 4; }
            constexpr u32 nblocks() const { return nblocks(mLen); }
        ...
        public:
            constexpr HashMurmurImpl(const T* data, const u32 len, const u32 seed = 0, void* userData = nullptr)
                : mData(data)
                , mLen(len)
                , mSeed(seed)
                , mUserData(userData) {
            ...
            constexpr void feed(const T* fedData, const u32 fedLen) {
                for (u32 i = 0; i < nblocks(fedLen); i++) {
        ...
        template <typename T, class Read>
        constexpr u32 hashMurmurImpl(const T* data, const u32 len, const u32 seed = 0, void* userData = nullptr) {
    ...
    template <typename T>
    constexpr u32 hashMurmur(const T* data, const u32 len, const u32 seed = 0) {
        static_assert(sizeof(T) == 1);
